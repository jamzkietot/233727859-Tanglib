...
document.addEventListener("DOMContentLoaded", () => {
    ...
    renderFleetOverview();
    // Live random drift disabled: each PC's status is computed once from the
    // table data and stays fixed from then on (healthy stays healthy,
    // warning stays warning, critical stays critical — no auto-flipping).
    // startLiveTicking();
});
 ...
 * Status is computed once here and never auto-changes afterward.
 * ==========================================================================
 */
function buildPcRegistry() {
    ...
    tableRows.forEach(row => {
        ...
        // Seed the sensor history buffer used by the gauge/chart dashboard.
        pc.history = [{ cpu: pc.cpuTemp, board: pc.boardTemp, storage: pc.computedStoragePercent }];

        // Event log — kept empty since status no longer auto-transitions.
        pc.eventLog = [];
...
/**
 * NOTE: The functions below (tickLiveData, pushPcEvent, logPcEventsIfChanged,
 * startLiveTicking) are kept in the file but are no longer invoked from
 * DOMContentLoaded. This is what previously caused a "healthy" PC to drift
 * into "warning"/"critical" a moment after load. Status is now computed
 * once in buildPcRegistry()/recomputeDerivedFields() and never changes
 * automatically again — healthy stays healthy, critical stays critical.
 */
...
function renderModalBody() {
    ...
    body.innerHTML = `
        ...
        <div class="live-pulse-indicator"><span class="live-dot"></span>Live monitoring snapshot</div>
        ...
        <div class="event-log-panel">
            ...
            <div class="event-log-list">
                ${pc.eventLog.length === 0
                    ? `<div style="color:var(--color-text-muted); font-size:12px; padding:6px 0;">No events recorded — monitoring nominal.</div>`
